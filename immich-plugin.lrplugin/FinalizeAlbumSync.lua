...
return {
    LrTasks.startAsyncTask(function()
        ...
        if #collections == 0 then
            LrDialogs.message("No collections", "No Immich published collections found.", "warning")
            return
        end
        ...
        if #collections == 1 then
            ...
            LrFunctionContext.callWithContext("selectCollection", function(context)
                ...
                local result = LrDialogs.presentModalDialog({
                    title = "Select Collection to Finalize",
                    contents = f:column {
                        bind_to_object = propertyTable,
                        f:static_text {
                            title = "Choose Immich published collection to finalize:",
                        },
                        f:popup_menu {
                            items = collections,
        ...
        -- Run finalize phase.
        local success, message = AlbumSync.finalizeImportedPhotos(pubCollection, stagingFolder)
        
        if success then
            LrDialogs.message("Album sync complete", message, "info")
        else
            LrDialogs.message("Album sync finalize failed", message, "critical")
        end
    end)
}
